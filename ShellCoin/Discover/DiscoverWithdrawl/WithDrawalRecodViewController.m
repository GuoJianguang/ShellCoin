...
@interface WithDrawalRecodViewController ()<UITableViewDelegate,UITableViewDataSource>
@property (nonatomic, assign)NSInteger page;
@property (nonatomic, strong)NSMutableArray *dataSouceArray;
@end

@implementation WithDrawalRecodViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.naviBar.title = @"提现记录";
    __weak WithDrawalRecodViewController *weak_self = self;
    ...
    [HttpClient POST:@"find/withdraw/get" parameters:prams success:^(NSURLSessionDataTask *operation, id jsonObject) {
        if (IsRequestTrue) {
            ...
            for (NSDictionary *dic in array) {
                WithDrawalRecodModel *model = [WithDrawalRecodModel modelWithDic:dic];
...
{
    WithDrawalRecodTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:[WithDrawalRecodTableViewCell indentify]];
    if (!cell) {
        cell = [WithDrawalRecodTableViewCell newCell];
    }
